...
WebSocket-клиент выбирает профиль Chrome, Firefox или Safari из нескольких
вариантов `User-Agent` и согласованных Client Hints; выбранный профиль
сохраняется на всё время логического туннеля. WS-сессия планово
закрывается через случайный интервал между `websocket_min_session_secs` и
`websocket_max_session_secs`, после короткой паузы подключается к той же ноде
и не переключает каскад failover. При этом клиент возобновляет прежний
`session_id`: VPN IP, TUN, маршруты и счётчик активных сессий сохраняются,
а меняются только WebSocket и транспортный ключ. Сервер отправляет ping с непостоянным
интервалом. На время переподключения payload health-check приостанавливается,
после resume включается снова. Для публичного размещения рекомендуется терминировать `wss://`
обычным nginx/Caddy на стандартном HTTPS-порту.
