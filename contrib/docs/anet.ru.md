...
## Тарифы и Управление Доступом (Enterprise Mode)

Если вы используете `anet-auth` (HTTP-бэкенд с базой данных), вам становится доступна гибкая система тарификации. В локальном режиме (`allowed_clients`) проверки лимитов нет — там доступ либо есть, либо его нет.

Логика тарифов завязана на связке **User <-> Rate**.
...
1. **Проверка при входе (`CheckAccess`):**
  ...
  * **Активен ли юзер?** (Флаг `is_active`). Если `false` — мгновенный бан.
  * **Не протух ли ключ?** (Поле `date_end` в таблице тарифов). Если текущее время больше даты окончания — отказ.
...
2. **Учет сессий:**
  ...
  * При штатном (или аварийном, благодаря RAII-гардам в коде) отключении клиента, сервер шлет `/session/stop`, и счетчик уменьшается (-1).
...
* `[CORE AUTH] Banned` — администратор (или жизнь) ограничил вам доступ.

### Управление через API

Поскольку `anet-auth` — это полноценный REST API, вы можете управлять тарифами на лету (например, прикрутив бота в Telegram или веб-морду).

**Пример PATCH-запроса для обновления тарифа юзера:**
```http
PATCH /api/v1/rate/{rate_id}
X-Admin-Token: <JWT_TOKEN>
Content-Type: application/json

{
  "sessions": 5,
  "date_end": "2026-12-31-23:59"
}
```
